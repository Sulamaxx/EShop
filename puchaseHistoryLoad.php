...
SELECT invoice.id AS iid, product.id AS pid, invoice_item.date AS bdate,product.title AS pname,invoice_item.qty AS bqty,invoice_item.balance AS cost
...
WHERE invoice.users_email='" . $user . "' ORDER BY invoice.id DESC
");
    $n = $result->num_rows;
    $total = 0;
    if ($n != 0) {
    ...
    ?>
            <tr class="border-bottom">
                <td class="col-2 p-2 border-right text-center"><?php echo $data["iid"]; ?></td>
                <td class="col-2 p-2 border-right text-center"><?php echo $data["bdate"] ?></td>
